...
export function useViewerProfiles(
  options: Omit<
    UseQueryOptions<Awaited<ReturnType<typeof readViewerProfiles>>, unknown>,
    "queryFn" | "queryKey"
  > = {},
) {
  const fetchViewerProfiles = useServerFn(getViewerProfiles);

  return useQuery({
    ...options,
    queryKey: ["viewer_profiles"],
    queryFn: () => fetchViewerProfiles(),
  });
}
...
export const profileQueryOptions = <Out>(
  id: number,
  options: Omit<
    UseQueryOptions<Awaited<ReturnType<typeof readProfile>>, unknown, Out>,
    "queryFn" | "queryKey"
  > = {},
) =>
  queryOptions({
