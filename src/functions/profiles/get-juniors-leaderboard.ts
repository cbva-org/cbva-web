...
export const getJuniorsLeaderboardFn = createServerFn()
	...
	.handler(
		async ({
			...
			if (gender) {
				rankConditions.push(eq(playerProfiles.gender, gender));
			}
			...
			const rankedSubquery = db
				.select({
					...
					odRank:
						sql<number>`dense_rank() over (order by ${playerProfiles.juniorsPoints} desc)`.as(
							"od_rank",
						),
				})
				.from(playerProfiles)
				...
				db
					.select({
						...
						collegeTeamYearsParticipated:
							playerProfiles.collegeTeamYearsParticipated,
						externalRef: playerProfiles.externalRef,
						level: {
							id: levels.id,
							name: levels.name,
							abbreviated: levels.abbreviated,
						},
					})
					.from(playerProfiles)
