...
function RouteComponent() {
	...
	const canShowTeams = hasTeams || canUpdate;

	useEffect(() => {
		if (!canShowTeams && tabParam === "teams") {
			navigate({
	...
	const canShowPools = hasPools;

	useEffect(() => {
		if (!canShowPools && tabParam === "pools") {
			navigate({
				to: "/tournaments/$tournamentId/$divisionId/{-$tab}",
				params: {
					tournamentId,
	...
	const canShowGames = hasGames;

	useEffect(() => {
		if (!canShowGames && tabParam === "games") {
			navigate({
				to: "/tournaments/$tournamentId/$divisionId/{-$tab}",
				params: {
					tournamentId,
	...
	}, [navigate, canShowGames, tabParam, tournamentId, divisionId]);
	...
	const canShowPlayoffs = hasGames;

	useEffect(() => {
		if (!canShowPlayoffs && tabParam === "playoffs") {
			navigate({
				to: "/tournaments/$tournamentId/$divisionId/{-$tab}",
				params: {
					tournamentId,
	...
	}, [navigate, canShowPlayoffs, tabParam, tournamentId, divisionId]);
